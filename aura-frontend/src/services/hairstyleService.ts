...
export const getHairstyles = async (category?: string): Promise<Hairstyle[]> => {
  ...
  const { data } = await api.get<Hairstyle[]>('/api/hairstyle', { params });
  return data;
};

export const getHairstyle = async (id: number): Promise<Hairstyle> => {
  const { data } = await api.get<Hairstyle>(`/api/hairstyle/${id}`);
  return data;
};

export const getCategories = async (): Promise<string[]> => {
  const { data } = await api.get<string[]>('/api/hairstyle/categories');
  return data;
};
...
export const validatePhoto = async (imageFile: File): Promise<{ isValid: boolean; issues: string[] }> => {
  ...
  const { data } = await api.post('/api/try-on/validate', formData, {
...
export const generateTryOn = async (
  ...
  const { data } = await api.post<TryOnJob>('/api/try-on/generate', formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  });
  return data;
};
...
export const getJobStatus = async (jobId: string): Promise<TryOnJob> => {
  const { data } = await api.get<TryOnJob>(`/api/try-on/${jobId}`);
  return data;
};

export const regenerateTryOn = async (jobId: string, hairstyleId?: number): Promise<TryOnJob> => {
  const { data } = await api.post<TryOnJob>('/api/try-on/regenerate', { jobId, hairstyleId });
  return data;
};
